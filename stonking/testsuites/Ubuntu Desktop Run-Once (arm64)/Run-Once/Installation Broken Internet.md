...
<dt>Boot up the iso using a DVD or USB Key to a Live Session</dt>
...
<dt>Select the Try FAMILY option</dt>
  <dd>Confirm after booting to live session that the relevant items are in the correct language, apps like 'Ubuntu Software' will still be referred to as 'Ubuntu Software'.</dd>
<dt>Click on the Install FAMILY desktop icon</dt>
<dt>Verify the language selected is the same as selected earlier on</dt>
