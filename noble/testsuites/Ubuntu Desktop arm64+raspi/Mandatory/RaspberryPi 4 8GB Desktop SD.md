...
Follow the installation steps at [ IoT installation media](<>), and write the image to an SD card. Then, using `sudo rpi-eeprom-config`, ensure the EEPROM's `[BOOT_ORDER](<>)` is set to 0xf1. 
