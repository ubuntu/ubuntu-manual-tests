...
Follow the installation steps at [ IoT installation media](<>), and write the image to the internal eMMC. Then, using `sudo rpi-eeprom-config`, ensure the EEPROM's `[BOOT_ORDER](<>)` is set to 0xf1. 

Before booting your CM4 with the new image, edit config.txt on the boot (1st) partition and uncomment the `#dtoverlay=dwc2,dr_mode=host` line to ensure the USB ports on the IO board operate correctly
...
Input your initial user details and password _admin_ can not be used - it is a dedicated Linux User 
...
Launch Settings from the menu that appears, then ""About"" in the left panel of the window that appears 
     Reported ""Memory"" is consistent with a Raspberry Pi Compute Module 4 4GB. It should be in the region of 3.6-3.8GB. 
