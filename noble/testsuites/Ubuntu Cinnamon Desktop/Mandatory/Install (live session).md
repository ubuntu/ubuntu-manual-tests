...
Press ""Try FAMILY"" and wait for the Live session to start
    The default desktop is displayed
Use the desktop and open some applications to try FAMILY before installing
Go to the Install FAMILY icon (usually on the desktop) and install from the Live session
...
Note the state of the 'Erase disk and install FAMILY' radio button
    The 'Erase disk and install FAMILY' radio button should be checked
Click on the continue button (if there is only one hard disk in the system, the button should read 'Install now')
    If there is only one hard disk, the installer should skip to the ""Where are you?' screen. Otherwise, the 'Installation type' screen is displayed
