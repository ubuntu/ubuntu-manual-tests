# PXE BIOS Netboot (Desktop Live)

The purpose of this testcase is to run netboot installation on amd64 machines
with **BIOS** firmware using live images. You need an installed Ubuntu machine
...
Update the pxelinux configuration in the netboot directory to point to ISO-URL:

1. sudo sed -i -e 's?iso-url=[^ ]*?iso-url=ISO_URL?' /srv/tftp/amd64/pxelinux.cfg/default
